...
if (invcusthoton = "1")
{
	...
	if RegisteredDirectory<>ERROR
	{
		if RegisteredDirectory>0
		{
			...
			{
				IniRead,CCDirectory%A_Index%,Settings.aldrin_ini,Edits,CCDirectory%A_Index%
				includesave:=CCDirectory%A_Index%
				IniRead,CCDirectory%A_Index%Enabled,Settings.aldrin_ini,Edits,CCDirectory%A_Index%Enabled
				IniRead,CCDirectory%A_Index%Hotkey,Settings.aldrin_ini,Edits,CCDirectory%A_Index%Hotkey
				includerparam:=CCDirectory%A_Index%Hotkey
				if CCDirectory%A_Index%Enabled=+
				{
					includewriter=%includewriter%%includerparam%::`r`nexecutinginclude=1`r`nif CCDirectory1Enabled=+`r`n{`r`nifinstring`,CCDirectory1Hotkey`,!`r`n{`r`nKeyWait`, Alt`r`n}`r`nifinstring`,CCDirectory1Hotkey`,^`r`n{`r`nKeyWait`, Ctrl`r`n}`r`nifinstring`,CCDirectory1Hotkey`,+`r`n{`r`nKeyWait`, Shift`r`n}`r`n#include *i %includesave%`r`n}`r`nexecutinginclude=0`r`cancelcast=0`r`nreturn`r`n`r`n
				}
			}
			ifexist,%A_ScriptDir%\execute.aldrin_includer
			{
				fileread,reader,%A_ScriptDir%\execute.aldrin_includer
				if reader<>%includewriter%
				{
					FileDelete,%A_ScriptDir%\execute.aldrin_includer
					FileAppend,%includewriter%,%A_ScriptDir%\execute.aldrin_includer
					Reload
					msgbox Reloading Execute
					sleep 1000
					msgbox Cannot Reload the Tool`,please manually reload this tool
				}
			}
			else
			{
...
loop,parse,tempparam,`,
{
	...
	IfInString,temphk, %existedvar%
	{
		...
		loop,parse,tempparam%integer%,`,
		{
			...
			if integer2=%A_Index%
			{
				includerparam=%includerparam%%existedvar2%::`r`n
			}
		}
	}
	else
...
StringReplace,mpocustbox,mpocustbox,`n,`nexecutinginclude=1`nif cancelcast=1`n{`r`ncancelcast=0`nreturn`n}`n,1
...
StringReplace,invocustbox,invocustbox,`n,`nexecutinginclude=1`nif cancelcast=1`n{`r`ncancelcast=0`nreturn`n}`n,1
...
If InStr(invfile,.aldrin_cc,-1)
{
	...
	StringReplace,invocustbox,invocustbox,`nexecutinginclude=1`nif cancelcast=1`n{`r`ncancelcast=0`nreturn`n}`n,`n,1
	GuiControl,Text,invshow,%invshow%
	GoSub,pgrmode
}
return
...
{
	FileRead,invshow,%invfile%
	StringReplace,invshow,invshow,`nexecutinginclude=1`nif cancelcast=1`n{`r`ncancelcast=0`nreturn`n}`n,`n,1
...
#include %A_ScriptDir%\execute.aldrin_includer
